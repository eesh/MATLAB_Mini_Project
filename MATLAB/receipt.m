...
guidata(hObject, handles);
    ...
    datestr = sprintf('Date: %d-%d-%d',dnt(3),dnt(2),dnt(1));
    if(dnt(4) >= 12)
        timestr = sprintf('Time: %d:%d pm',dnt(4),dnt(5));
    else
        timestr = sprintf('Time: %d:%d am',dnt(4),dnt(5));
    end
    
    set(findobj('Tag','text6'),'String',datestr);
    set(findobj('Tag','text7'),'String',timestr);
    global menulist;
    global x1;
    x1=10;
    global x2;
    ...
    tstr = sprintf('Rs. %d',TOTAL_BILL');
    uicontrol('Style','text','String',tstr,'Position',[x4-5 y-5 52 20]);
    y=y-25;
    uicontrol('Style','text','String','------------------------------------------------------------------','Position',[10 y 260 15]);
